...
export default function ClaimBuilder() {
  ...
  // HIPAA: uploaded documents are session-only. They remain available while the
  // builder is in use, but are wiped from local storage when the builder/tab is
  // closed so no document data is persisted on the device after the session ends.
  // (Logout also clears these via AuthContext.)
  useEffect(() => {
    ...
    window.addEventListener("beforeunload", clearUploadedDocuments);
    return () => {
      window.removeEventListener("beforeunload", clearUploadedDocuments);
    };
  }, []);
  ...
  // Normalize any SSN (formatted or raw) to a consistent XXX-XX-XXXX display.
  // profile.ssn from sessionStorage is already formatted, so we strip non-digits
  // first to avoid double-formatting (e.g. "123--45--6789").
  const formatSSNDisplay = (raw: string) => {
    const digits = (raw || "").replace(/\D/g, "");
    if (digits.length < 9) return digits ? raw : "XXX-XX-XXXX";
    return `${digits.slice(0, 3)}-${digits.slice(3, 5)}-${digits.slice(5, 9)}`;
  };
  const getSSNLast4 = (raw: string) => {
    const digits = (raw || "").replace(/\D/g, "");
    return digits.length >= 4 ? digits.slice(-4) : "XXXX";
  };

  // Current VA service-connected rating (set on the Medical History page).
  const getServiceConnectedInfo = () => {
    const connected = localStorage.getItem("serviceConnected") === "yes";
    const percentage = localStorage.getItem("serviceConnectedPercentage") || "";
    return { connected, percentage };
  };

  // Build a sentence summarizing service dates + current rating for the intro paragraph.
  const buildServiceSummary = () => {
    const si = getServiceHistory();
    const { connected, percentage } = getServiceConnectedInfo();
    const branch = getBranchName(si.branch);
    const entered = si.dateEntered ? String(si.dateEntered).trim() : "";
    const separated = si.dateSeparated ? String(si.dateSeparated).trim() : "";
    let summary = "";
    if (entered && separated) {
      summary = ` I served in the ${branch} from ${entered} to ${separated}.`;
    } else if (entered) {
      summary = ` I served in the ${branch} beginning ${entered}.`;
    }
    if (connected && percentage) {
      summary += ` I am currently service-connected at a combined rating of ${percentage}%.`;
    }
    return summary;
  };
  ...
  const processClaimData = async () => {
    ...
    try {
      ...
      const ssnFormatted = profile.ssn ? formatSSNDisplay(profile.ssn) : "";
                      ...
                      <div className="space-y-6 print:p-0">
                        ...
                        <div className="rounded-lg p-6 bg-white print:p-0 print:block print:break-before-page" data-testid="container-claim-memorandum" style={{ fontFamily: 'Arial', fontSize: '11pt', color: 'black' }}>
                          {generatedMemorandum ? (
                            <div className="text-base" style={{ lineHeight: 2, marginLeft: 0, paddingLeft: 0, color: 'black', fontFamily: 'Arial', fontSize: '11pt', border: 'none', borderWidth: 0 }}>
                              ...
                              {(() => {
                                ...
                                const ssnFormatted = formatSSNDisplay(profile.ssn);
                          ...
                          (() => {
                            ...
                            const last4SSN = getSSNLast4(profile.ssn);
